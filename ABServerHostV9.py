...
s = socket.socket()
while True:
    if NetworkCheck('192.168.84.1') == True:
        s.bind(('192.168.84.25',port))
        break
    else:
        print("Failed to bind to 192.168.84.25:2075")
        time.sleep(10)
utf = "utf-8"
os.chdir('/home/' + host + '/Documents/Programs/AutoBackup/')

def FolderSweeperRunner():
    FolderDirectories = []
    FolderContents = {}
    def FolderSweeper(RecursiveFolders): #Returns list with depth of folders and directories in the form of a list, starts alternating between directory (1st) to depth
        if RecursiveFolders == 'None':
            RecursiveFolders = ""
        for folder in os.listdir('/home/cat/Toshiba3TB/AutoBackup/' + RecursiveFolders):
            #If it DOESN'T find a . in the filename
            if folder.find('.') == -1:
                FolderDirectories.append(RecursiveFolders + folder + "/")
                FolderContents[folder] = os.listdir('/home/cat/Toshiba3TB/AutoBackup/' + RecursiveFolders + folder + "/")
                #FolderDirectories.append(str(RecursiveFolders + folder + "/").count('/'))
                for subfolder in os.listdir('/home/cat/Toshiba3TB/AutoBackup/' + RecursiveFolders + folder):
                    if subfolder.find('.') == -1:
                        PreviousFolders = RecursiveFolders + folder + "/" + subfolder + "/"
                        FolderDirectories.append(PreviousFolders)
                        FolderContents[subfolder] = os.listdir('/home/cat/Toshiba3TB/AutoBackup/' + RecursiveFolders + folder + "/" + subfolder + "/")
                        #FolderDirectories.append(str(PreviousFolders).count('/'))
                        FolderSweeper(PreviousFolders)
        return FolderContents
    return FolderSweeper('None')
