...
for command in BackupList:
    ...
    if os.path.isdir(StrippedCommand) == True:
        ...
        for command in CommandList:
            ...
            SendMsg('192.168.84.25', Checksum, 'm', user, SourceFolder + RefinedGazeHistory + FileName)
            message, FileName, address, ArgInfo = RecMsg()
            if message.decode(utf) == "approved":
                SendMsg('192.168.84.25', str(command), 'f', user, SourceFolder, GazeHistory[sync])
                print('\n')
                sync = sync + 1
            else:
                print(message)
        ...
        SendMsg('192.168.84.25', StrippedCommand, 'f', user + "/", "default")
